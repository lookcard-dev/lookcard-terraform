resource "aws_security_group" "authentication_api_security_grp" {
  depends_on  = [var.vpc_id]
  name        = "${local.application.name}-sg"
  description = "Security group for ${local.application.name}"
  vpc_id      = var.vpc_id
...
resource "aws_security_group" "authentication_api_ecs_svc_sg" {
