resource "aws_security_group" "profile-api-sg" {
  ...
  dynamic "ingress" {
    ...
    content {
      ...
      security_groups = [var.sg_alb_id, var.referral_api_sg, var.verification_api_sg, var.lambda_firebase_authorizer_sg_id, var.reseller_api_sg, var.bastion_sg] #var._auth_api_sg
    }
  }
