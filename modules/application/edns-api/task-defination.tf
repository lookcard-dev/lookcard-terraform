resource "aws_ecs_task_definition" "edns_api" {
  family                   = local.application.name
  network_mode             = "awsvpc"
  # requires_compatibilities = ["FARGATE"]
  cpu                      = "256"
  memory                   = "512"
  task_role_arn            = aws_iam_role.edns_api_task_role.arn
  execution_role_arn       = aws_iam_role.edns_api_task_execution_role.arn
  runtime_platform {
  ...
  container_definitions = jsonencode([
    {
      ...
      portMappings = [
        {
          name          = "look-card-edns-api-8080-tcp",
