resource "aws_ecs_task_definition" "referral_api" {
  family                   = local.application.name
  network_mode             = "awsvpc"
  cpu                      = "256"
  memory                   = "512"
  task_role_arn            = aws_iam_role.referral_api_task_role.arn
  execution_role_arn       = aws_iam_role.referral_api_task_execution_role.arn
  runtime_platform {
