resource "aws_ecs_task_definition" "reap_webhook" {
  family                   = local.application.name
  network_mode             = "awsvpc"
  # requires_compatibilities = ["FARGATE"]
  cpu                      = "256"
  memory                   = "512"
  task_role_arn            = aws_iam_role.reap_webhook_task_role.arn
  execution_role_arn       = aws_iam_role.reap_webhook_task_execution_role.arn
  runtime_platform {
