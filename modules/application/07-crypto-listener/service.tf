...
resource "aws_ecs_service" "ethereum_sepolia_blast_ecs_service" {
  count           = var.runtime_environment == "develop" || var.runtime_environment == "testing" ? 1 : 0
  name            = "ethereum-sepolia_blast"
  task_definition = aws_ecs_task_definition.ethereum_sepolia_blast_task_definition[0].arn
  desired_count   = var.image_tag == "latest" ? 0 : 1
  cluster         = var.cluster_id
  capacity_provider_strategy {
    capacity_provider = "LISTENER_EC2_AMD64"
...
resource "aws_ecs_service" "bsc_testnet_blast_ecs_service" {
  count           = var.runtime_environment == "develop" || var.runtime_environment == "testing" ? 1 : 0
  name            = "bsc-testnet_blast"
  task_definition = aws_ecs_task_definition.bsc_testnet_blast_task_definition[0].arn
  desired_count   = var.image_tag == "latest" ? 0 : 1
  cluster         = var.cluster_id
  capacity_provider_strategy {
    capacity_provider = "LISTENER_EC2_AMD64"
...
resource "aws_ecs_service" "arbitrum_sepolia_blast_ecs_service" {
  count           = var.runtime_environment == "develop" || var.runtime_environment == "testing" ? 1 : 0
  name            = "arbitrum-sepolia_blast"
  task_definition = aws_ecs_task_definition.arbitrum_sepolia_blast_task_definition[0].arn
  desired_count   = var.image_tag == "latest" ? 0 : 1
  cluster         = var.cluster_id
  capacity_provider_strategy {
    capacity_provider = "LISTENER_EC2_AMD64"
...
resource "aws_ecs_service" "base_sepolia_blast_ecs_service" {
  count           = var.runtime_environment == "develop" || var.runtime_environment == "testing" ? 1 : 0
  name            = "base-sepolia_blast"
  task_definition = aws_ecs_task_definition.base_sepolia_blast_task_definition[0].arn
  desired_count   = var.image_tag == "latest" ? 0 : 1
  cluster         = var.cluster_id
  capacity_provider_strategy {
    capacity_provider = "LISTENER_EC2_AMD64"
