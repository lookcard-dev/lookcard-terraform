...
resource "aws_ecs_service" "arbitrum_sepolia_publicnode_ecs_service" {
  count           = var.runtime_environment == "develop" || var.runtime_environment == "testing" ? 1 : 0
  name            = "arbitrum-sepolia_publicnode"
  task_definition = aws_ecs_task_definition.arbitrum_sepolia_publicnode_task_definition[0].arn
  desired_count   = var.image_tag == "latest" ? 0 : 1
  cluster         = var.cluster_id
  capacity_provider_strategy {
    capacity_provider = "LISTENER_EC2_AMD64"
