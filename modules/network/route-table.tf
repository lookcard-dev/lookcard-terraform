...
resource "aws_route_table" "private-route-table" {
  ...
  tags = {
    Name = "private-subnet-route-table-${aws_subnet.private-subnet[count.index].availability_zone}"
  }
}

resource "aws_route_table_association" "private_subnet_association" {
  count          = length(aws_subnet.private-subnet) # Ensure count matches the number of subnets
