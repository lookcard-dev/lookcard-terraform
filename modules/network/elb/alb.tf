...
resource "aws_lb_listener" "application_load_balancer_listener" {
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.application_load_balancer_default_target_group.arn
  }
}

resource "aws_lb_target_group" "application_load_balancer_default_target_group" {
  name        = "alb-default-tg"
  port        = 80
  protocol    = "HTTP"
  target_type = "ip"
